...
class DocentesXGrupos
{
    ...
    public static function new()
    {
        try {
            ...
            $sentence = $db->prepare("
                INSERT INTO docentes_x_grupos (id_docente, id_grupo, es_titular, activo, fecha_asignacion) 
                VALUES (:id_docente, :id_grupo, :es_titular, 1, :fecha_asignacion)
            ");
            $sentence->bindParam(':id_docente', $id_docente);
            $sentence->bindParam(':id_grupo', $id_grupo);
    ...
    public static function updateTitular()
    {
        try {
            ...
            if ($es_titular) {
                ...
                $getGrupo = $db->prepare("SELECT id_grupo FROM docentes_x_grupos WHERE id = :id");
